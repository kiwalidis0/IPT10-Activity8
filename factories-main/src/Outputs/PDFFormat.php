...
class PDFFormat implements ProfileFormatter
{
    ...
    public function setData($profile)
    {
        ...
        // Certifications
        $this->pdf->Cell(0,10, 'Certifications: ');
        $this->pdf->Ln();
        foreach ($profile->getCertifications() as $certification) {
            $this->pdf->Cell(0, 10, '- ' .$certification['name'] . ' during ' . $certification['date_earned'], 0, 1);
        }

        // Extra-Curricular Activites
        $this->pdf->Cell(0, 10, 'Extra-Curricular Activities:', 0, 1);
        foreach ($profile->getExtraCurricularActivities() as $activity){
            $this->pdf->Cell(0, 10, '- ' . $activity['role'] . ' at ' . $activity['organization'] . ' (From: ' . $activity['start_date'] . ' to ' . $activity['end_date'] . ')', 0, 1);
